 ...
 * including the MIT license.
 */
package org.spout.vanilla.entity.state;
 ...
 */
public class VanillaPlayerInputState implements PlayerInputState{
	...
	public static final PlayerInputState DEFAULT_STATE = new VanillaPlayerInputState((short) 0, (byte) 0, (byte) 0);
	private final short userCommands;
	private final float pitch;
	private final float yaw;

	public VanillaPlayerInputState(boolean forward, boolean backward, boolean left, boolean right, boolean jump, boolean crouch, boolean selectUp, boolean selectDown, boolean fire1, boolean fire2, boolean interact, float pitch, float yaw) {
	...
	public VanillaPlayerInputState(short userCommands, float pitch, float yaw) {
		this.userCommands = userCommands;
		this.pitch = pitch;
		this.yaw = yaw;
	}
	...
	public PlayerInputState withAddedFlag(Flags flag) {
		return new VanillaPlayerInputState((short) (userCommands | flag.getBitFlag()), pitch, yaw);
	}
	...
	public PlayerInputState withAddedPitch(float pitch) {
		return new VanillaPlayerInputState(userCommands, MathHelper.wrapAnglePitch(pitch + this.pitch), yaw);
	}

	public PlayerInputState withAddedYaw(float yaw) {
		return new VanillaPlayerInputState(userCommands, pitch, MathHelper.wrapAngle(yaw + this.yaw));
	}
	...
	@Override
	public PlayerInputState reset() {
		return new VanillaPlayerInputState((short) 0, pitch, yaw);
	}

	@Override
	public void process(Entity entity) {
		Transform ts = entity.getTransform().getTransform();
		...
		entity.getTransform().setTransform(ts);
	}
}
