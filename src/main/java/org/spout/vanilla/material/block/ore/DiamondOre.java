...
public class DiamondOre extends Ore implements TimedCraftable, Mineable {
	...
	public ArrayList<VanillaItemStack> getDrops(Block block) {
		...
		if (block.getSource() instanceof Entity) {
			...
			if (held != null && held.getMaterial().equals(VanillaMaterials.IRON_PICKAXE, VanillaMaterials.DIAMOND_PICKAXE) && held.hasEnchantment(Enchantments.SILK_TOUCH)) {
				drops.add(new VanillaItemStack(this, 1));
			} else {
				drops.add(new VanillaItemStack(VanillaMaterials.DIAMOND, 1));
			}
		}
		return drops;
	}
