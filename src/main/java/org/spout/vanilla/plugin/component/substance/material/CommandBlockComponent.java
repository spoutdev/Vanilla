...
public class CommandBlockComponent extends VanillaBlockComponent implements CommandSource {
	...
	public void onAttached() {
		super.onAttached();
		this.activeChannel.set(Engine.STANDARD_BROADCAST_CHANNEL);
	}
	...
	@Override
	public boolean hasPermission(World world, String perm) {
		return true;
	}
	...
	public void processCommand(String command, ChatArguments arguments) {
		...
		Command cmd = CommandBlock.getRootCommand().getChild(command, false);
		if (cmd != null) {
	...
	private List<Player> getAllMatchingPlayers(char[] match) {
		List<Player> matching = new ArrayList<Player>();
		Filter filter = getFilter(match);
	...
	private Player getNearestMatchingPlayer(char[] match) {
		List<Player> matching = new ArrayList<Player>();
		Filter filter = getFilter(match);
		...
		Spout.log(result.getName());
		
		return result;
	}
	
	private Player getRandomMatchingPlayer(char[] match) {
		List<Player> players = getAllMatchingPlayers(match);
		if (players.isEmpty()) {
			return null;
		}
			...
			if (str[i] == '@' && i + 1 < str.length) {
				...
				if (str[i] == 'p') { // nearest player
					...
					if (i + 1 < str.length && str[i + 1] == '[') {
						...
						Spout.log("char: " + str[i + size] + "");
						if (str[i + size] == ']') {
							filter = new char[--size];
							System.arraycopy(str, i + 1, filter, 0, filter.length);
							i += size + 1;
							
							Spout.log("buffer: " + new String(filter));
						}
						regex = "@p\\[[^\\]]+\\]";
					}
					
					Player player = getNearestMatchingPlayer(filter);
					if (player != null) {
					...
					Player player = getRandomMatchingPlayer(filter);
					if (player != null) {
						arguments = ChatArguments.fromFormatString(arguments.toFormatString().replaceFirst(regex, player.getName()));
						result.addAll(getAllPossibleCommandArguments(arguments));
					}
	...
	public boolean sendRawMessage(ChatArguments message) {
		getActiveChannel().broadcastToReceivers(new ChatArguments(ChatStyle.GRAY, ChatStyle.ITALIC, "[@: ", message, "]"));
		return true;
	}
