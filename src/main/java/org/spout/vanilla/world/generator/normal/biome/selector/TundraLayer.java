...
public class TundraLayer implements BiomeSelectorLayer {
	...
	public TundraLayer(double scale) {
	...
	public BiomeSelectorElement pick(int x, int y, int z, long seed) {
		...
		final BiomeSelectorElement element = NormalBiomeSelector.FROZEN_RIVER.pick(x, y, z, seed);
		if (element == null) {
			final BiomeSelectorElement ocean = NormalBiomeSelector.FROZEN_OCEAN.pick(x, y, z, seed);
			if (ocean == null) {
				final float value = (float) hills.GetValue(x, 500, z);
				if (value > HILLS_START
						&& value <= HILLS_END) {
					return VanillaBiomes.TUNDRA_HILLS;
				} else {
