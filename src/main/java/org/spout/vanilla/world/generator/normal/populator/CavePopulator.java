...
public class CavePopulator extends OverlapingPopulator {
	...
	private static class CaveNode {
		...
		private CaveNode(CuboidBlockMaterialBuffer blockData2, Vector3 chunk, Vector3 start, Vector3 end, Vector3 target,
				...
				for (int z = start.getFloorZ(); z < end.getFloorZ(); z++) {
					...
					for (int y = end.getFloorY() - 1; y >= start.getFloorY(); y--) {
						...
						if (yOffset > -0.7 && xOffset * xOffset + yOffset * yOffset + zOffset * zOffset < 1) {
							...
							if (id == VanillaMaterials.STONE || id == VanillaMaterials.DIRT
									|| id == VanillaMaterials.GRASS) {
								if (y < 10) {
									blockData.set(xx, y, zz, VanillaMaterials.LAVA);
								} else {
									if (id == VanillaMaterials.GRASS
											&& blockData.get(xx, y - 1, zz) == VanillaMaterials.DIRT) {
